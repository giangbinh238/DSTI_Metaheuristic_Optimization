...
class SimulatedAnnealing:
    ...
    def PrintConvergence(self):
        ...
        ax1.plot(self.temperature,self.convergence, color = "orange")
        ax1.set_title("Fitness vs. Temperature",fontsize=16)
        ax1.set_xlabel("Temperature",fontsize=14)
        ax1.set_ylabel(" Best fitness (total distance)",fontsize=14)
        ax1.set_xlim(self.start_temp, self.current_temp)

        ax2.plot(np.arange(1,self.epochs +1,1),self.convergence)
        ax2.set_title("Fitness vs. Fevals",fontsize=16)
        ax2.set_xlabel("Number of function evaluations",fontsize=14)
        #ax2.set_ylabel("Fitness(total distance)",fontsize=14)
